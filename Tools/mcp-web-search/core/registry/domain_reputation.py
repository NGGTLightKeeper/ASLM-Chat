...
class DomainReputationStore:
    ...
    def get_promoted_tier(self, domain: str) -> Optional[str]:
        """Return auto-promoted tier ('C' or 'B') if earned, else None."""
        conn = self._connect()
        try:
            row = conn.execute(
                "SELECT promoted_tier FROM domain_decisions WHERE domain=?",
                (domain,),
            ).fetchone()
            return row["promoted_tier"] if row else None
        finally:
            conn.close()

    def get_report(self, domain: str) -> Optional[DomainReport]:
        """Return a full reputation summary for a domain, or None if unseen."""
        conn = self._connect()
        try:
            rows = conn.execute(
            ...
            global_obs = 0
            for r in rows:
                if r["query_type"] == _GLOBAL:
                    global_ema = r["ema_score"]
                    global_obs = r["obs_count"]
                else:
                    per_type[r["query_type"]] = {
                        "ema": round(r["ema_score"], 4),
                        "obs": r["obs_count"],
                    }
            ...
            blacklisted = bool(dec["auto_blacklisted"]) if dec else False
            blacklisted_at = dec["blacklisted_at"] if dec else None
            promoted_tier = dec["promoted_tier"] if dec else None
            promoted_at = dec["promoted_at"] if dec else None
            protected = bool(dec["protected"]) if dec else False
            try:
                promoted_types = json.loads(
                    (dec["promoted_query_types"] if dec else "[]") or "[]"
                )
            except Exception:
                promoted_types = []
            ...
            return DomainReport(
                ...
                promoted_query_types=promoted_types,
                protected=protected,
            )
        finally:
            conn.close()
    ...
    def top_blacklisted(self, limit: int = 20) -> list[dict]:
        """Return top auto-blacklisted domains sorted by EMA score ascending."""
        conn = self._connect()
        try:
            rows = conn.execute(
                """
                SELECT d.domain, d.blacklist_ema, d.blacklist_obs, d.blacklisted_at,
                       r.ema_score AS current_ema
                FROM domain_decisions d
                LEFT JOIN domain_reputation r ON r.domain = d.domain AND r.query_type = ?
                WHERE d.auto_blacklisted = 1
                ORDER BY d.blacklist_ema ASC
                LIMIT ?
                """,
                (_GLOBAL, limit),
            ).fetchall()
    ...
    def top_promoted(self, limit: int = 20) -> list[dict]:
        """Return top promoted domains sorted by EMA score descending."""
        conn = self._connect()
        try:
            rows = conn.execute(
                """
                SELECT d.domain, d.promoted_tier, d.promoted_at, d.promoted_query_types,
                       r.ema_score AS global_ema, r.obs_count AS global_obs
                FROM domain_decisions d
                LEFT JOIN domain_reputation r ON r.domain = d.domain AND r.query_type = ?
                WHERE d.promoted_tier IS NOT NULL
                ORDER BY r.ema_score DESC
                LIMIT ?
                """,
                (_GLOBAL, limit),
            ).fetchall()
