...
Public API
----------
SourceCache          -- main class, owns the DB
...
logger = logging.getLogger("source_cache")

# ---------------------------------------------------------------------------
# Reuse URL helpers from crawl_frontier when available, else inline fallback.
# ---------------------------------------------------------------------------
try:
    from deep_research.src.background_agent.crawl_frontier import (
        canonicalize_url,
        url_hash,
        content_hash,
    )
    ...
    try:
        ...
        # Inline fallback so the module works standalone.
...
# No triggers needed — FTS5 is updated explicitly in cache_page().
_TRIGGERS_SQL = ""
...
class SourceCache:
    ...
    def cache_page(
        ...
        try:
            ...
            # Explicitly maintain FTS5 index (UPSERT doesn't fire triggers).
            conn.execute(
                "DELETE FROM pages_fts WHERE url_hash = ?", (uhash,)
            )
            conn.execute(
    ...
    def record_query_source(self, query: str, url: str, rank: int) -> None:
        ...
        try:
            conn.execute(
                ...
                (query.strip().lower(), uhash, rank, now),
            )
            conn.commit()
        finally:
            conn.close()

    def evict_stale(self, max_age_sec: int = 604800) -> int:
