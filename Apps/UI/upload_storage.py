...
def _stored_file_name(file_id: str, original_name: str) -> str:
    ...
    short_id = re.sub(r"[^a-zA-Z0-9-]+", "", str(file_id or ""))[:STORED_FILE_ID_CHARS] or "upload"
    return f"{short_id}__{safe_name}"
...
def load_upload_manifest(file_id: str) -> dict[str, Any] | None:
    ...
    if not normalized_id:
        return None
    sidecar_patterns = [
        f"**/{normalized_id[:STORED_FILE_ID_CHARS]}__*{UPLOAD_MANIFEST_SUFFIX}",
        f"**/{normalized_id}__*{UPLOAD_MANIFEST_SUFFIX}",
    ]
    for pattern in sidecar_patterns:
        for sidecar in USER_UPLOAD_ROOT.glob(pattern):
            try:
                manifest = json.loads(sidecar.read_text(encoding="utf-8"))
            except (OSError, json.JSONDecodeError):
                continue
            if str(manifest.get("file_id") or "") == normalized_id:
                return manifest
    return None
